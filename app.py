...
with app.app_context():
    try:
        ...
        try:
            ...
            if not admin_user:
                admin_user = models.User(username='admin', name='Administrador', role='admin', email='admin@senai.br')
                admin_user.set_password('senai103103')
                db.session.add(admin_user)
                db.session.commit()
                logging.info("Admin user created successfully! Username: admin | Password: senai103103")
            else:
